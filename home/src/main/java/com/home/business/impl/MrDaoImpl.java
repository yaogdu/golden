  ...
  public List<MarketingResearch> CustomizedMr(long ts, long uid) {
    Query query =
        em.createNativeQuery(
            "select * from t_marketing_research where mr_id in (select uh_id from t_user_history where uh_type=1  and u_id=?1 and uh_status!=5 and uh_id>?2 order by uh_id desc  ) order by mr_id desc",
            AppPromotion.class);
