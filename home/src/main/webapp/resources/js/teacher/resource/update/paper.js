latte.teacher.paper = {
	...
	init : function() {
		latte.teacher.paper.postData.id = $("#id").val();
	...
	initPaper : function() {
		$.ajax({
			...
			dataType : 'json',
			data : {
				"resourceId" : latte.teacher.paper.postData.id,
				"resourceType" : 3
			},
			success : function(response) {
				...
				$('#paper_file_name').text(data.fileName);
				$("#resourceFormat").val(
				...
				latte.teacher.paper.postData.resourceName = data.resourceName;
				$('.infoTextarea').val(data.intro);
				latte.teacher.paper.postData.intro = data.intro;
				 
				latte.teacher.paper.postData.shareLevel=data.shareLevel;
				...
				if(sharelevel==2){
					...
					//$('#file_ico').attr('class','ico-'+util.appFormatSearch(util.getFormatParent(util.getTail(data.fileName))));
					//latte.teacher.paper.postData.resourceFormat=util.getFormatParent(util.getTail(data.fileName));
					$('#stage').val(data.stage.id);
					latte.teacher.paper.postData.stage=data.stage;
					latte.teacher.paper.initStage($('#stage').val());
					
					$('#subject').val(data.subject.id);
					latte.teacher.paper.postData.subject=data.subject;
					
					$('#paperType').val(data.paperType);
					latte.teacher.paper.postData.paperType=data.paperType;
					$('#paperYear').val(data.paperYear);
					latte.teacher.paper.postData.paperYear=data.paperYear;
					
					
					$('#testArea').val(data.testArea);
					latte.teacher.paper.postData.testArea=data.testArea;
					...
					if(data.testArea!=6){
						...
						latte.teacher.paper.postData.paperArea=data.paperArea;
						latte.teacher.paper.getProvince();
						/*$('#provSel').val(latte.teacher.paper.data.prov);
						latte.teacher.paper.initPaperArea();*/
					}
					$('#grade').val(data.grade);
					latte.teacher.paper.postData.grade=data.grade;
					$('#term').val(data.term);
					latte.teacher.paper.postData.term=data.term; 
				}else{
...
//			}
		...
		latte.teacher.paper.fileUploader = new plupload.Uploader(
				{
					...
					url : ctx + "/teacher/testpaper/update",
					...
					init : {
						...
						FilesAdded : function(up, file) {
							...
							latte.teacher.paper.updateFile = true;
						},
						BeforeUpload : function(up, file) {
							latte.teacher.paper.fileUploader.disableBrowse(true);
							$('#btn_save_paper').css('disabled', '');// 禁用保存按钮
						},
	...
	commit : function() {// 提交
		...
		var b = latte.teacher.paper.updateFile;
		...
		if (b) {
			...
			latte.teacher.paper.fileUploader.settings.url = ctx
					+ "/teacher/testpaper/update";// 拼接上传url与param
			...
			$.ajax({
				type : 'POST',
				contentType : 'application/json',
				url : ctx + "/teacher/testpaper/updateWithoutFile",
				dataType : "json",
				data : JSON.stringify(latte.teacher.paper.postData),
				success : function(response) {
					alert("成功");
				},
