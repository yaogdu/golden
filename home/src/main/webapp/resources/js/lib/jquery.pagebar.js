    ...
    $.extend($.fn, {        
        jpagebar: function(setting) {
			...
			pb.resetPagebar = function(){	
				...
				$('<div>共&nbsp;'+_this.totalpage+'页&nbsp;,&nbsp;'+_this.totalcount+'&nbsp;条记录</div>').appendTo(render_to);
				var pagebar = $('<ul></ul>');
				...
				if(_this.totalpage == 1){//共1页
					$('<li><a href="javascript:void(0);">1</a></li>').attr('class',_this.currentPageNumberCssName).bind("click", function(){_this.onClickPage(1);}).appendTo(_this.renderTo);
				}
				else{//有多页
				...
				if(_this.currentPage == 1 ){//当前页为第一页
					$('<li><a href="javascript:void(0);">上一页</a></li>').attr('class',_this.pageNameCssName + ' ' + _this.currentPageNameCssName).prependTo(pagebar);
				}
				else{
					//当前页大于第一页
					$('<li><a href="javascript:void(0);">上一页</a></li>').attr('class',_this.pageNameCssName)
						 .bind("click", function(){_this.onClickPage(_this.currentPage-1);}).prependTo(pagebar);
				}
				 //处理Next
				if (_this.currentPage == _this.totalpage) {//当前页为最后一页 
					$('<li><a href="javascript:void(0);">下一页</a></li>').attr('class',_this.pageNameCssName + ' ' + _this.currentPageNameCssName).appendTo(pagebar);
				} else {   
					$('<li><a href="javascript:void(0);">下一页</a></li>').attr('class',_this.pageNameCssName)
				...
				$('#go_pg_num').bind('input propertychange',function(){
					...
					if(currentPage == ''){
					}else if(isNaN(currentPage) || currentPage<1 ||( _this.totalpage && _this.totalpage < currentPage)){
						 $(this).val(1);//不符合要求则置1
					}
				});
				
				$('.page-btn').click(function(){
					var goPgNum = $('#go_pg_num').val();
