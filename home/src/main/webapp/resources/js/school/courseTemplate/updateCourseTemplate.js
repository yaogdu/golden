$(document).ready(function(){
	...
	$("div[id^='chapter_modify_']").each(function(){
		$(this).hide();
	});
	$("div[id^='sectionLeft_modify_']").each(function(){
		$(this).hide();
	});
	$("div[id^='sectionRight_show_']").each(function(){
		$(this).hide();
	});
	...
	$("a[id^='chapter_img_']").each(function(){
		var id = $(this).attr("id");
		var cid = id.substring(12,id.length);
		$("#chapter_img_" + cid).bind("click",{id: cid}, showChapterMenu);
	});
	$("a[id^='section_img_']").each(function(){
		var id = $(this).attr("id");
		var sid = id.substring(12,id.length);
		$("#section_img_" + sid).bind("click",{id: sid}, showSectionMenu);
	});
...
function update()
{
	...
	var url = $("#context").val() + "/school/ct/update";
        ...
        ok : function() {
        	...
        	$.ajax({
        		...
        		success: function(data){
        			if( data.result == true )
        			{
        				util.dialog.messageAndRelocation('设置完毕',$("#context").val() + "/school/ct/toListPage");
        				return true;
        			}
        			else
        			{
